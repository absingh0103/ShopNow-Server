...
const TextEffect1 = () => {
    ...
    return (
        <div className="h-full flex ">
            <h1 className="text-rose-500 text-xl font-extrabold font-mono relative inline-block">
                <span id="before-te1" className="absolute top-0 left-0.5 w-full h-full bg-transparent" style={{ textShadow: "-2px 0 #49FC00", clipPath: "rect(24px, 550px, 90px, 0)" }} aria-hidden="true">Featured Products</span> {/* glitch::before */}
                Featured Products
                <span id="after-te1" className="absolute top-0 -left-0.5 w-full h-full bg-transparent" style={{ textShadow: "-2px 0 spin(#49FC00, 180)", clipPath: "rect(85px, 550px, 140px, 0)" }} aria-hidden="true">Featured Products</span> {/* glitch::after */}
            </h1>
        </div>
    )
}
